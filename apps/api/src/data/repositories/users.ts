...
import { parseLinkedEmails, parseLinkedForms, stringifyLinkedEmails, stringifyLinkedForms } from '@/lib/models';
...
export class UsersRepositoryImplementation implements UsersRepository {
  ...
  async delete(uid: Uid) {
    const userExists = await this.db.select().from(users).where(eq(users.id, uid.value));
    if (!userExists[0]) return Response.error('User not found.', 404);

    const deleted = await this.db.delete(users).where(eq(users.id, uid.value)).returning();
  ...
  async updateName(uid: Uid, newName: Name) {
    ...
    const updated = await this.db.updated(users).set({name: newName.value}).where(eq(users.id, uid.value)).returning();
  ...
  async updateEmail(uid: Uid, newEmail: Email) {
    ...
    const updated = await this.db.updated(users).set({email: newEmail.value}).where(eq(users.id, uid.value)).returning();
    if (!updated[0]) return Response.error('Internal error.')
  ...
  async updatePassword(uid: Uid, newPassword: Password) {
    ...
    const updated = await this.db.updated(users).set({password: newHashedPassword}).where(eq(users.id, uid.value)).returning();
    if (!updated[0]) return Response.error('Internal error.')
  ...
  async updateLinkedEmails(uid: Uid, linkedEmails: Email[]) {
    ...
    const updated = await this.db.updated(users).set({linkedEmails: _linkedEmails}).where(eq(users.id, uid.value)).returning();
    if (!updated[0]) return Response.error('Internal error.')
  ...
  async save(name: Name, email: Email, password: Password) {
    ...
    const newUser = await this.db.insert(users).values({ name: name.value, email: email.value, linkedEmails, forms, password: hashedPassword }).returning();
