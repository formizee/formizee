...
export class EndpointsRepositoryImplementation implements EndpointsRepository {
  ...
  async load (uid: Uid) {
    const response = await this.db.select().from(endpoints).where(eq(endpoints.id, uid.value))
    ...
    const endpoint = new Endpoint(response[0].id, response[0].name, response[0].owner, response[0].targetEmail, response[0].redirectUrl, submissions);
    return Response.success(endpoint);
  }

  async loadByOwner (owner: Uid) {
    const response = await this.db.select().from(endpoints).where(eq(endpoints.owner, owner.value))

    if (!response[0]) return Response.error('The user does not have forms.', 404);
  ...
  async save (name: Name, owner: Uid) {
    const ownerData = await this.db.select().from(users).where(eq(users.id, owner.value))
    if (!ownerData[0]) return Response.error('The user does not exist.', 404);

    const response = await this.db.insert(endpoints).values({ name: name.value, owner: owner.value, targetEmail: ownerData[0].email}).returning();

    const endpoint = new Endpoint(response[0].id, response[0].name, response[0].owner, response[0].targetEmail, response[0].redirectUrl, []);
    return Response.success(endpoint);
  }
