...
import { DeleteSubmission, LoadSubmission, LoadSubmissionByForm, SaveSubmission } from "@/useCases/submissions";

export const router = new Hono();

router.get('/:form', async context => {
  const { isAuth, user } = await verifySession(context);
  if (!isAuth || !user) return context.json({ error: "Please, login first in order to do this action" }, 401);
  ...
  if(!endpointResponse.ok) return context.json(endpointResponse.error, endpointResponse.status as StatusCode);

  if (endpointResponse.body.owner !== user) return context.json({error: "You do not have permission to access this data"}, 401);
...
router.delete('/:uid', async context => {
  ...
  if(!submissionsResponse.ok) return context.json(submissionsResponse.error, submissionsResponse.status as StatusCode);
