...
export async function encrypt(context: Context, payload: VerificationPayload): Promise<string> {
...
export async function decrypt(context: Context, data: string | undefined = ''): Promise<JWTPayload | undefined> {
  const env = context.env as Env
...
export async function readVerification(context: Context): Promise<{isValid: boolean, email: string | undefined, type: 'account' | 'password'}> {
  ...
  return {isValid: true, email: data.email as string, type: data.type === 'account' ? 'account' : 'password'};
}
