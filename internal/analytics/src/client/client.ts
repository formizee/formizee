  ...
  >(req: {
    ...
    return async (
      ...
      if (validParams?.error) {
        return Err(
          new QueryError(`Bad params: ${validParams.error.message}`, {
            query: ''
          })
        );
      }
      ...
      try {
        ...
        return Err(
          new QueryError(`Unable to query clickhouse: ${message}`, {
            query: req.query
          })
        );
      }
      ...
      if (parsed.error) {
        return Err(
          new QueryError(`Malformed data: ${parsed.error.message}`, {
            query: req.query
          })
        );
      }
